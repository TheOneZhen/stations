class Paster {
  ...
  public static paste() {
    ...
    const instance = this
    this.getImagePath(filePath, selectText, this.folderPathConfig, this.showFilePathConfirmInputBox, this.filePathConfirmInputBoxMode, (err, imagePath) => {
      try {
        // is the file existed?
  ...
  public static getImagePath(filePath: string, selectText: string, folderPathFromConfig: string, showFilePathConfirmInputBox: boolean, filePathConfirmInputBoxMode: string, callback: (err, imagePath: string) => void) {
    // image file name
    let imageFileName = ''
    if (!selectText) {
